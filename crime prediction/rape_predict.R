...
library(ggplot2)
ggplot() +
  geom_point(aes(x = training1_set$Year, y = training1_set$Rape),
             colour = 'red') +
  geom_line(aes(x = training1_set$Year, y = predict(poly_reg, newdata = training1_set)),
            colour = 'blue') +
  ggtitle('Rape vs Year (bihar)(saharsa)') +
  xlab('Years') +
  ylab('Rape')
...
write.csv(test_set,"rape_bihar(sahara).csv")