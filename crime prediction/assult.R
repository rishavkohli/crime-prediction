...
model_set<-cbind(model_set,dataset[,8])
...
library(ggplot2)
ggplot() +
  geom_point(aes(x = training1_set$Year, y = training1_set$assult),
             colour = 'red') +
  geom_line(aes(x = training1_set$Year, y = predict(poly_reg, newdata = training1_set)),
            colour = 'blue') +
  ggtitle('assult vs Year (bihar)(saharsa)') +
  xlab('year') +
  ylab('assult')
...
write.csv(test_set,"assult_bihar(sahara).csv")
