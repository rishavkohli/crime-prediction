...
model_set<-cbind(model_set,dataset[,7])
...
library(ggplot2)
ggplot() +
  geom_point(aes(x = training1_set$Year, y = training1_set$dowry),
             colour = 'red') +
  geom_line(aes(x = training1_set$Year, y = predict(poly_reg, newdata = training1_set)),
            colour = 'blue') +
  ggtitle('dowry vs Year (bihar)(saharsa)') +
  xlab('Years') +
  ylab('dowry')
...
write.csv(test_set,"dowry_bihar(sahara).csv")
