...
model_set<-cbind(model_set,dataset[,6])
...
poly_reg = lm(formula = kidnapping ~ .,
              data = train_set)
summary(poly_reg)
...
library(ggplot2)
ggplot() +
  geom_point(aes(x = training1_set$Year, y = training1_set$kidnapping),
             colour = 'red') +
  geom_line(aes(x = training1_set$Year, y = predict(poly_reg, newdata = training1_set)),
            colour = 'blue') +
  ggtitle('kidnapping vs Year (bihar)(saharsa)') +
  xlab('Years') +
  ylab('kidnapping')
...
write.csv(test_set,"kidnapping_bihar(sahara).csv")
